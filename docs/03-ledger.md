...
fold(events):
    ...
    return RUNNING(current, cycles, attempts)
```
...
## Grounding a gate in something the worker didn't write

Every field above except one passed through the worker's session before it
reached the ledger: the summary is the worker's, the artifact paths are claimed
by the worker's `transition` call, the proposal is the worker's. So none of
them can, on their own, answer "did it actually pass?" — a worker that wants to
be done will write whatever it takes.

The exception is `guard_checked.check_output`. A transition's `:check` is a
command the **harness** runs, in its own subprocess, after the stage exits, with
no agent anywhere in the path. Exit 0 passes the edge. That is the mechanism
that keeps "did it pass?" grounded in a real exit code rather than the worker's
optimism (see [07-risks.md](07-risks.md)).

Composed with it, `criteria` is graded by an independent Judge that sees the
worker's output digest, its artifacts, and the check's stdout — never the
worker's claim of success. Check first, and a failed check is not appealable to
the Judge; spending a spawn to second-guess a non-zero exit can only weaken the
gate.

An earlier design had a third channel, `vars_set`, carrying "trusted" variables
scraped off tool stdout for Fennel `when` guards to gate on. It was not
trusted: the scrape ran over *every* tool result, so any stage with `bash`
could print the marker itself. The tier is gone. A ledger written before its
removal still folds — the fields it dropped are simply ignored.
...
- The `open-pr` stage checks for an existing PR for the branch before opening one.
...
{"ts":"2026-07-23T18:00:02Z","type":"state_entered","state":"implement","cycle":1,"attempt":1,"session_id":"01f9…","model":"claude-sonnet-5","thinking":"high","skills":["spark-build"]}
...
{"ts":"2026-07-23T18:08:10Z","type":"guard_checked","from":"implement","to":"review","structural":"pass","check":"pass","criteria":"pass","check_output":"build ok","judge_rationale":"All three checklist items present in diff; no TODO markers."}
