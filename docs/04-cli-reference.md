...
Seven subcommands: [`init`](#loop-init), [`validate`](#loop-validate),
[`diagram`](#loop-diagram), [`run`](#loop-run), [`resume`](#loop-resume),
[`status`](#loop-status), [`doctor`](#loop-doctor).

For what the runtime actually does with the machine, see
[02-how-it-works.md](02-how-it-works.md). For the keys inside `config.fnl` and
`machine.fnl`, see [03-customizing.md](03-customizing.md).
...
`--dir` is global: it may appear before or after the subcommand. It sets
`project_dir`, which anchors `.loop/`, the ledger, artifacts, and the cwd of
...
Runs two scaffold phases: the global toolbox (idempotent, safe to re-run), then
the project's `.loop/`. Every file is written with a *write-if-absent* rule —
**nothing existing is ever overwritten**, with one exception noted below.
...
**Exception to write-if-absent:** the three `ext/*.ts` are written by
`materialize_ext()`, which compares the on-disk sha256 against the copy compiled
into the binary and **rewrites the file on mismatch**. Hand edits to
`ext/transition-tool.ts`, `ext/verdict-tool.ts`, or `ext/choose-tool.ts` are
silently reverted — by `loop init` and also by every `loop run` / `loop resume`.

**Phase 2 — project** (`<project>/.loop/`). Bails first if the machine already
exists:
...
Otherwise reads `<config_dir>/machines/<template>.fnl` (missing → `no machine
template at <path>`) and writes:
...
All three files get a plain `str::replace("$TICKET", <ticket>)` — a literal text
substitution, **not** the `$VAR` render engine used at runtime. No other
placeholder is expanded at init time.
...
Loads `config.fnl` and `machine.fnl`, resolves every playbook and skill against
the toolbox, and prints one line per diagnostic in the form:
...
- **Warnings alone exit 0**, but suppress the "no problems found" line — a
  warning-only run prints only the `warn ` lines.
- Reachability walks only edges into *defined states*, starting at `entry`; it
...
Writes mermaid to stdout with no code fences and no surrounding prose, so it
pipes directly:
...
Output is deterministic and a pure function of the machine IR — it touches no
toolbox files, so a machine with a dangling `:playbook` or an unresolvable skill
still draws. It still requires the machine to load: a missing or unparseable
`machine.fnl` is an error.

For the node/edge/label grammar (aliasing, edge labels, guard-fail back-edges,
loop notes), see [02-how-it-works.md](02-how-it-works.md).
...
`--max-transitions` **only tightens**: it is merged with the effective budget by
per-field minimum, so a value larger than the machine's own `max-transitions` has
no effect. It cannot relax `usd` or `wallclock-s` either way.
...
`{Status}` is `Done`, `Failed`, or `Aborted`. `{terminal}` falls back to
`(no terminal)`. Cost is two decimal places. Duration formatting:
...
The final summary line is printed *before* the error, so a failed run emits both.
...
the ledger precondition, which is inverted:
...
Reads the ledger and folds it. Does not require the machine to load — if
`machine.fnl` is missing or mid-edit, status still works; it only loses the
`cycles:` line (and cycle counting falls back to treating every state as a loop
head).
...
Header is one of `not started`, ``running — at `{state}` ``, or
`finished — {Status}`. The `cycles:` line appears only when the machine loaded
and at least one cycle was counted. `recent:` lists the last 12 ledger events,
oldest-first within that window. Per-event summary forms:
...
All four pass → blank line, then `all good`, exit 0. Otherwise exit 1 with
`error: {n} problem(s)` on stderr.
...
- Check 3 probes **only `ext/transition-tool.ts`**. A missing or corrupt
  `verdict-tool.ts` or `choose-tool.ts` still reports `ok`.
...
1. `LOOP_CONFIG_DIR` / `LOOP_STATE_DIR`, taken verbatim (no expansion, no
   absoluteness requirement).
2. `$XDG_CONFIG_HOME/loop` / `$XDG_STATE_HOME/loop`, **only if the XDG value is
   an absolute path**; a relative value is ignored entirely.
3. `$HOME/.config/loop` / `$HOME/.local/state/loop`, only if `$HOME` is set and
   non-empty.
4. The relative paths `.config/loop` / `.local/state/loop`, resolved against the
   process cwd.
...
- **`LOOP_PI_BIN` cannot be set from `config.fnl`.** Config loading passes the
  binary through untouched, so the environment variable is the only lever.
- **`LOOP_STATE_DIR` does not move the ledger.** The ledger is always
  `<project>/.loop/ledger.jsonl`. `LOOP_STATE_DIR` only relocates the rendered
  system prompts under `render/`.
...
There are no other exit codes: `main` catches a single error type, prints
`error: {e:#}` to stderr, and exits 1. The `{e:#}` form prints the full
context chain, so a failure surfaces as `error: outer: inner: root cause`.
...
`loop run` and `loop resume` exit 1 on `Failed` and `Aborted` deliberately, so
`loop run && gh pr merge` and CI wrappers gate correctly. To distinguish the two,
read `status` from [`loop status --json`](#loop-status).
