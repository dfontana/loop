...
The most recent `qa` failure detail (error_class `real`) is in the digest, with a
job-log artifact. Start there, not from the top.
...
3. **Fix the actual cause.** Common real failures on this pipeline:
   - `column X not found in gold schema` → the migration was authored but never
     ...
     the migration file). Register it.
   - nulls written for the new column → the job isn't computing/writing it, or is
...
5. Finish with `transition(to="qa_staging", rationale="<root cause → the fix>",
   ...
   `transition(blocked=true, rationale=…)` rather than papering over it.

## Notes
- One concrete fix per cycle. If you're changing three unrelated things, you
  haven't found the root cause yet.
- The next stage is read-only QA against staging — leave the tree deployable and
  the migration registered so QA can actually observe your fix.
