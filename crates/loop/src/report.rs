...
fn fmt_outcome(rs: &RunState) -> String {
    match rs.fold_status() {
        crate::core::FoldStatus::NotStarted => "not started".into(),
        crate::core::FoldStatus::Running => format!(
            "unfinished — last at `{}`",
            rs.current.as_deref().unwrap_or("?")
        ),
        crate::core::FoldStatus::Finished(s) => format!(
            "finished — {s:?} at `{}`",
            rs.current.as_deref().unwrap_or("?")
        ),
    }
...
mod tests {
    use super::*;
    use crate::core::{Budgets, ErrorKind, RunStatus};
