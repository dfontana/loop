...
use crate::core::{
    Artifact, Budgets, Check, Context, Event, EventPayload, GuardOutcome, LoopSpec, Machine,
    ModelSpec, OnExhausted, OnFail, ResumePoint, RunFinish, RunStart, RunState, StagePromptRef,
    StateId, Transition, Usage,
};
use crate::engine::Diagnostic;
use crate::toolbox::render;

use crate::core::text::{first_line, truncate};
use crate::episode::Episode;
use crate::output::{fmt_duration, fmt_status, fmt_totals, summarize};
...
/// The string both renderers accumulate into, and the one piece of rendering
/// they genuinely share.
///
/// [`Columns`] and [`Markdown`] stay separate *renderers* — mixing them
/// produces a file that is neither aligned nor valid Markdown, and merging
/// them into one parameterised type does not survive contact with the
/// parameters, since a column report indents and a Markdown report nests
/// headings by level. But the buffer underneath was declared twice, with two
/// `new`s, two `Default`s and two `finish`es, and `labelled` took a
/// `&mut String` first argument purely because there was no shared type to
/// hang it on.
#[derive(Default)]
...
impl Buf {
    ...
    /// Write a labelled value, one line per line of it: `first` before the
    /// first, `rest` before every continuation.
    ///
    /// Both renderers need a multi-line value to stay attached to its label — a
    /// prose `:criteria` inside preview's value column, a Judge rationale
    /// inside recap's bullet — and both need an empty value to read as
    /// [`NONE`] rather than as a line that failed to render. Only the two
    /// prefixes differ, and both are the caller's.
    fn labelled(&mut self, value: &str, first: &str, rest: &str) {
        let value = if value.trim().is_empty() { NONE } else { value };
        for (i, line) in value.lines().enumerate() {
            let prefix = if i == 0 { first } else { rest };
            let _ = writeln!(self.0, "{prefix}{line}");
        }
    }
}

/// The aligned-column report: `loop preview`'s format.
#[derive(Default)]
pub struct Columns {
    out: Buf,
}
...
impl Columns {
    ...
    pub fn field_at(&mut self, indent: usize, label: &str, value: impl AsRef<str>) {
        ...
        self.out
            .labelled(value.as_ref(), &first, &" ".repeat(VALUE_COL));
    }
    ...
    /// A field whose value may be absent, rendered as [`NONE`] when it is.
    ///
    /// `labelled` already turns an empty value into `(none)`, so the six
    /// callers that reached it through `.clone().unwrap_or_default()` were
    /// paying an allocation and three lines of rustfmt to say `Option`.
    pub fn field_opt(&mut self, label: &str, value: Option<impl AsRef<str>>) {
        self.field_opt_at(2, label, value);
    }

    /// [`Columns::field_opt`] at an explicit indent.
    pub fn field_opt_at(&mut self, indent: usize, label: &str, value: Option<impl AsRef<str>>) {
        self.field_at(
            indent,
            label,
    ...
    pub fn block(&mut self, indent: usize, text: &str) {
        for line in text.lines() {
            if line.is_empty() {
                ...
                let _ = writeln!(self.out.0, "{:indent$}{line}", "", indent = indent);
            }
        }
    }
...
/// The Markdown report: `loop recap`'s format.
///
/// `loop recap > run-recap.md` is in the command's contract, so recap's
/// structure is real Markdown — headings, bullets, fences — rather than
/// [`Columns`]' alignment, which collapses into one paragraph the moment a
/// renderer touches it. The labels and their order stay shared with preview;
/// only the punctuation around them differs.
#[derive(Default)]
pub struct Markdown {
    out: Buf,
}
...
impl Markdown {
    ...
    pub fn bullet(&mut self, label: impl AsRef<str>, value: impl AsRef<str>) {
        let first = format!("- {}: ", label.as_ref());
        self.out.labelled(value.as_ref(), &first, "  ");
    }
    ...
    pub fn fence(&mut self, text: &str) {
        ...
        let _ = writeln!(self.out.0, "{fence}");
        for line in text.lines() {
            let _ = writeln!(self.out.0, "{line}");
        }
        let _ = writeln!(self.out.0, "{fence}\n");
    }
    ...
    fn ensure_blank(&mut self) {
        if self.out.0.is_empty() || self.out.0.ends_with("\n\n") {
            return;
        }
        if !self.out.0.ends_with('\n') {
            self.out.0.push('\n');
        }
        self.out.0.push('\n');
    }
...
pub fn machine_preview(r: &Resolver<'_>) -> String {
    ...
    rep.field_opt("escalation", m.escalation_state.as_deref());
...
fn state_block(rep: &mut Columns, r: &Resolver<'_>, id: &StateId) {
    let m = r.machine;
    let description = m
        .state(id)
...
fn edge_fields(rep: &mut Columns, t: &Transition) {
    match &t.check {
...
pub fn state_preview(r: &Resolver<'_>, id: &StateId) -> String {
    ...
    rep.field_opt("description", resolved.stage_prompt.description.as_deref());
    rep.field_opt("model", resolved.stage_prompt.model.as_deref());
    rep.field_opt(
        "thinking",
        resolved.stage_prompt.thinking.map(|t| t.to_string()),
    );
    ...
    rep.field("cwd", display(&paths.project_dir));
    rep.field(
        "system prompt",
        display(&paths.render_file_pattern(id, "system")),
    );
    rep.field("env", crate::stage::ENV_VARS.join(", "));
...
#[derive(Default)]
struct Timeline<'e> {
    started: Option<RunStart<'e>>,
    finished: Option<RunFinish<'e>>,
...
fn timeline<'e>(events: &'e [Event]) -> Timeline<'e> {
    // Both brackets come off the shared destructurers in `core`, which pick the
    // outermost pair. That leaves `brackets` below with nothing to do but
    // *recognize* the two lines, so no attempt reports one as its own — it is a
    // plain predicate rather than a closure threading `&mut Timeline` through
    // two loops to fill a field on the way past.
    let mut t = Timeline {
        started: crate::core::run_started(events),
        finished: crate::core::run_finished(events),
        ..Timeline::default()
    };

    let brackets = |e: &Event| {
        matches!(
            e.payload,
            EventPayload::RunStarted { .. } | EventPayload::RunFinished { .. }
        )
    };

    let episodes = crate::episode::episodes(events);
    let first = episodes.first().map_or(events.len(), |ep| ep.ordinal);
    t.prelude = events[..first].iter().filter(|e| !brackets(e)).collect();

    for ep in episodes {
        let body: Vec<&Event> = ep.body.iter().filter(|e| !brackets(e)).collect();
...
pub enum Provenance {
    ...
    ///
    /// Owned, so `commands::recap` can settle provenance in one `match` over
    /// the two hashes instead of binding the loaded machine first and matching
    /// a second time to wrap it. Boxed because a `Machine` dwarfs the other
    /// two variants.
    Matches(Box<Machine>),
...
impl Provenance {
    ...
    /// The machine, only when it is provably the one that ran. Folding a ledger
    /// against a machine that has since been edited would report cycles the run
    /// never had.
...
fn run_summary(rep: &mut Markdown, r: &Recap<'_>, t: &Timeline<'_>) {
    ...
    rep.bullet("outcome", fmt_status(r.folded));
...
fn attempt_timeline(rep: &mut Markdown, r: &Recap<'_>, t: &Timeline<'_>) {
    ...
    for (n, (ep, body)) in t.attempts.iter().enumerate() {
        ...
        rep.bullet(
            "model",
            fmt_recorded_model(&ep.header.model, &ep.header.thinking),
        );
        rep.bullet("skills", join(&ep.header.skills));
        rep.bullet("mcp", join(&ep.header.mcp));
        rep.bullet(
            "session",
            ep.session()
...
fn episode_event(rep: &mut Markdown, e: &Event) {
    match &e.payload {
        ...
        EventPayload::TransitionProposed {
            ...
            rep.para(format!("**Proposal** by the Worker — `{from}` {target}:"));
...
fn why_it_ended(rep: &mut Markdown, r: &Recap<'_>, t: &Timeline<'_>) {
    ...
    if f.status != crate::core::RunStatus::Done {
...
fn inspection_pointers(rep: &mut Markdown, t: &Timeline<'_>) {
    ...
    for (ep, _) in t.attempts.iter().filter(|(e, _)| e.session().is_some()) {
        *by_state.entry(ep.state()).or_insert(0) += 1;
    }
...
fn last_fatal(events: &[Event]) -> Option<&str> {
    crate::core::last(events, |e| match &e.payload {
        EventPayload::Error {
            kind: crate::core::ErrorKind::Fatal,
            detail,
            ..
        } => Some(detail.as_str()),
        _ => None,
...
fn fmt_artifacts(artifacts: &[Artifact]) -> String {
...
/// `anthropic/claude-sonnet-5:high` — provider, model and thinking.
fn fmt_model(m: &ModelSpec) -> String {
    format!("{}/{}", m.provider, m.pi_model_arg())
}

/// The same, from the two strings a `state_entered` line records.
///
/// The ledger keeps the model and the thinking level apart and never records
/// the provider, so a recap cannot render [`fmt_model`]'s full form — but it
/// can at least use the same `model:thinking` spelling rather than its own
/// `format!` five hundred lines away.
fn fmt_recorded_model(model: &str, thinking: &str) -> String {
...
///
/// Emptiness is the *item count*, not the joined string: testing the string
/// meant a one-element list holding `""` reported as `(none)`, which is a
/// different claim — "this state names no skills" rather than "this state
/// names one with a blank name".
fn join(items: impl IntoIterator<Item = impl AsRef<str>>) -> String {
    let parts: Vec<String> = items.into_iter().map(|i| i.as_ref().to_string()).collect();
    if parts.is_empty() {
        NONE.into()
    } else {
        parts.join(", ")
...
mod tests {
    ...
    use crate::core::fixtures::{EventExt, entered, note, started as started_for};
    ...
    fn output(state: &str, cycle: u32, summary: &str) -> Event {
        crate::core::fixtures::output(state, cycle).summary(summary)
    }

    fn finished() -> Event {
        crate::core::fixtures::finished(RunStatus::Done, "done")
    }

    fn recap_of(events: &[Event]) -> String {
        recap(&Recap {
            events,
            folded: &crate::core::fold(events),
    ...
    fn the_run_brackets_are_lifted_out_of_the_episodes() {
        let events = vec![started(), entered("a", 1, 1), finished()];
        let t = timeline(&events);
    ...
    fn events_before_the_first_attempt_are_kept() {
        let events = vec![started(), note("staging the toolbox"), entered("a", 1, 1)];
        let t = timeline(&events);
    ...
    fn an_attempt_that_produced_nothing_still_gets_a_section() {
        let events = vec![
            started(),
            entered("implement", 1, 1).session(Some("T-1-implement-1-1")),
            crate::core::fixtures::error("implement", "the spawn died").fatal(),
        ];
    ...
    fn an_attempt_without_a_session_says_so_rather_than_omitting_the_line() {
        let events = vec![started(), entered("implement", 1, 1).session(Some("   "))];
        let out = recap_of(&events);
    ...
    fn the_same_ledger_renders_the_same_report() {
        let events = vec![
            started(),
            entered("implement", 1, 1).session(Some("s")),
