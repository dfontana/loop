...
fn happy_path_entry_to_terminal_with_right_ledger_sequence() {
    ...
    for i in 0..2 {
        if let crate::core::EventPayload::GuardChecked {
            structural,
            criteria,
            ..
        } = ledger.payloads_of("guard_checked")[i]
        {
            assert_eq!(*structural, crate::core::GuardOutcome::Pass);
            assert_eq!(*criteria, crate::core::GuardOutcome::Pass);
        } else {
...
fn judge_fail_retries_source_state() {
    ...
    let attempts: Vec<u32> = entered
        .iter()
        .map(|p| match p {
            crate::core::EventPayload::StateEntered { attempt, .. } => *attempt,
            _ => unreachable!(),
        })
        .collect();
    ...
    match checks[0] {
        crate::core::EventPayload::GuardChecked { criteria, .. } => {
            assert_eq!(*criteria, crate::core::GuardOutcome::Fail)
        }
        _ => unreachable!(),
    }
    match checks[1] {
...
fn judge_fail_with_on_fail_route_sends_elsewhere_without_further_guard_checks() {
    ...
    m.states.insert("implement".into(), state("implement"));
    m.states.insert("review".into(), state("review"));
    m.transitions.push(crate::core::Transition {
    ...
    match commits[0] {
        crate::core::EventPayload::TransitionCommitted { to, .. } => {
...
fn a_failing_check_blocks_the_edge_and_never_reaches_the_judge() {
    ...
    match ledger.payloads_of("guard_checked")[0] {
        crate::core::EventPayload::GuardChecked {
            check,
            criteria,
            check_output,
            ..
        } => {
            assert_eq!(*check, crate::core::GuardOutcome::Fail);
            assert_eq!(*criteria, crate::core::GuardOutcome::Skip);
...
fn a_passing_check_still_defers_to_the_judge_and_hands_it_the_output() {
    ...
    match ledger.payloads_of("guard_checked")[0] {
        crate::core::EventPayload::GuardChecked {
            ...
            assert_eq!(*check, crate::core::GuardOutcome::Pass);
            assert_eq!(*criteria, crate::core::GuardOutcome::Fail);
        }
        _ => unreachable!(),
    }
...
fn an_edge_without_a_check_skips_the_tier() {
    ...
    match ledger.payloads_of("guard_checked")[0] {
        crate::core::EventPayload::GuardChecked {
            ...
            assert_eq!(*check, crate::core::GuardOutcome::Skip);
...
fn transient_vs_real_routing_drives_self_loop_then_real_failure_to_debug() {
    ...
    let cycles: Vec<(String, u32)> = entered
        .iter()
        .map(|p| match p {
            crate::core::EventPayload::StateEntered { state, cycle, .. } => (state.clone(), *cycle),
            _ => unreachable!(),
        })
        .collect();
    assert_eq!(
...
fn max_cycles_exhaustion_escalates() {
    ...
    let qa_entries = entered
        .iter()
        .filter(
            |p| matches!(p, crate::core::EventPayload::StateEntered{state,..} if state=="qa_staging"),
        )
        .count();
...
fn validate_catches_unresolved_stage_prompt() {
    ...
    m.states.insert("a".into(), state("a"));
    m.transitions.push(edge("a", "done"));
    let diags = crate::engine::validate(&m, &never_resolves, &|_| true);
...
fn validate_rejects_duplicate_edges_between_the_same_pair() {
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.severity == crate::engine::Severity::Error
...
fn validate_reports_a_skill_that_does_not_resolve() {
    ...
    let diags = crate::engine::validate(&m, &always_resolves, &|name| name != "contract-check");
    assert!(
        diags
            .iter()
            .any(|d| d.severity == crate::engine::Severity::Error
                && d.message.contains("contract-check")),
        "got: {diags:?}"
    );
}
...
fn validate_checks_skills_that_come_from_the_machine_defaults() {
    ...
    let diags = crate::engine::validate(&m, &always_resolves, &|name| name != "hose-typo");
    let d = diags
        .iter()
        .find(|d| d.message.contains("hose-typo"))
        .unwrap_or_else(|| panic!("expected a diagnostic for the default skill: {diags:?}"));
    assert_eq!(d.severity, crate::engine::Severity::Error);
    assert!(
...
fn validate_reports_named_mcp_servers_without_the_mcp_extension() {
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.severity == crate::engine::Severity::Error
                && d.message.contains("pi-extensions")),
        "got: {diags:?}"
    );
    ...
    let ok = crate::engine::validate(&m, &always_resolves, &|_| true);
...
fn validate_warns_on_an_edge_with_no_check_and_no_criteria() {
    ...
    assert!(
        diags
            .iter()
            .any(|d| d.severity == crate::engine::Severity::Warning
...
fn validate_counts_an_on_fail_route_as_loop_head_re_entry() {
    ...
    m.loops.push(crate::core::LoopSpec {
        name: "fix".into(),
...
fn an_unresolvable_artifact_claim_is_recorded_and_dropped_not_fatal() {
    ...
    match ledger.payloads_of("worker_output")[0] {
        crate::core::EventPayload::WorkerOutput { artifacts, .. } => {
    ...
    assert_eq!(errors.len(), 1);
    match errors[0] {
        crate::core::EventPayload::Error { detail, .. } => {
...
fn the_judges_usage_lands_on_the_guard_event_and_in_the_totals() {
    ...
    runner.script_judge(crate::core::Verdict {
        pass: true,
    ...
    match ledger.payloads_of("guard_checked")[0] {
        crate::core::EventPayload::GuardChecked { usage, .. } => {
